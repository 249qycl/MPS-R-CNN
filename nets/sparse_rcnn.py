...
class DynamicHead(nn.Module):
    ...
    def forward(self, x, init_boxes, init_params, shapes):
        ...
        :param init_boxes:[num_proposal,4] 提议框
...
class SparseRCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        init_boxes_cxcywh = self.init_proposal_boxes.weight * self.shape_weights
        init_boxes_xyxy=box_convert(init_boxes_cxcywh,'cxcywh','xyxy')
        cls_predicts, box_predicts = self.head({"p2": p2, "p3": p3, "p4": p4, "p5": p5},
                                               init_boxes_xyxy,
                                               self.init_proposal_features.weight,
                                               shapes)
        return cls_predicts, box_predicts,shapes

    def loss_compute(self,cls_predicts, box_predicts, targets, shapes):
        ret = dict()
...
def post_process(cls_predict, box_predict, shapes,**cfg):
    ...
    for score, box, shape in zip(scores, box_predict, shapes):
        scores_per_image, topk_indices = score.flatten(0, 1).topk(cfg['num_proposals'], sorted=False)
