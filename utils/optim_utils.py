...
def split_params(model: nn.Module):
    param_other, param_weight_decay, param_bias = list(), list(), list(
...
def split_params_v2(model: nn.Module):
    param_other, param_weight, param_bias, param_norm = list(), list(), list(
...
def split_optimizer_v2(model: nn.Module, cfg: dict):
    ...
    optimizer.add_param_group({'params': param_norm, 'weight_decay': 0.})
    return optimizer
...
class IterWarmUpCosineDecayMultiStepLRAdjust(object):
    def __init__(self,
        ...
        self.gamma = gamma
        self.alpha = alpha
        last_epoch = epochs + 1 if len(
            milestones) > 0 and milestones[-1] == epochs else epochs
        self.flag = np.array([warm_up_epoch] + self.milestones +
                             [last_epoch]).astype(np.int)
    ...
    def cosine(self, current, total):
        return ((1 + math.cos(current * math.pi / total)) /
                2)**self.gamma * (1 - self.alpha) + self.alpha

    def get_lr(self, ite, epoch):
        current_iter = self.iter_per_epoch * epoch + ite
        if epoch < self.warm_up_epoch:
            up_lr = np.interp(current_iter, [0, self.warm_up_iter],
                              [0, self.init_lr])
            down_lr = np.interp(current_iter, [0, self.warm_up_iter],
                                [0.1, self.init_lr])
            return up_lr, down_lr
        num_pow = (self.flag <= epoch).sum() - 1
        multi_step_weights = self.alpha**num_pow  #衰减权重
        if num_pow == len(self.flag) - 2:
            lr = multi_step_weights * self.init_lr
            return lr, lr
        cosine_ite = (epoch - self.flag[num_pow]) * self.iter_per_epoch + ite
        cosine_all_ite = (self.flag[num_pow + 1] -
                          self.flag[num_pow]) * self.iter_per_epoch
    ...
    def __call__(self, optimizer, ite, epoch):
        ...
        for i, param_group in enumerate(optimizer.param_groups):
            param_group[
                'lr'] = dlr if self.bias_idx is not None and i == self.bias_idx else ulr
        return ulr, dlr
...
class IterWarmUpMultiStepDecay(object):
    def __init__(self,
        ...
        assert warm_up_iter < self.ite_mile_stones[0] and self.ite_mile_stones[
            -1] <= self.epochs * self.iter_per_epoch

    def get_lr(self, ite, epoch):
        current_iter = self.iter_per_epoch * epoch + ite
        if current_iter <= self.warm_up_iter:
            lr = np.interp(current_iter, [0, self.warm_up_iter],
                           [self.init_lr * self.warm_up_factor, self.init_lr])
        else:
