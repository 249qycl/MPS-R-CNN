...
class BoxInfo(object):
    ...
    def load_img(self):
        # key_name=os.path.basename(self.img_path)
        # if key_name in BoxInfo.img_dict.keys():  
        #     self.img=BoxInfo.img_dict[key_name]
        # else:          
        self.img = cv.imread(self.img_path)
            # BoxInfo.img_dict.update({key_name:self.img})        
        return self
